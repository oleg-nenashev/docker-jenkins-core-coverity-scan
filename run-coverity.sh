...
printHelp() {
  echo "Runs Coverity Scan for Jenkins Maven project."
...
# First run, which prepares temp snapshots
#mvn -DskipTests=true -Dfindbugs.skip=true install || echo "Full copilation was not successful, will try the core"

# Prepare build dir
cd core

# Run build with maven
cov-build --dir cov-int mvn -DskipTests=true -Dfindbugs.skip=true clean compile 
